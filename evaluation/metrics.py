...
# def calculate_metrics_per_video(predictions, labels, config):
#     """
#     计算每个受试者的心率估计MAE（基于心率而非波形，使用calculate_metric_per_video）。
#     Args:
#         predictions: dict, {subj: {seg: tensor}}
#         labels: dict, {subj: {seg: tensor}}
#         config: 配置对象，需包含TEST.DATA.FS和TEST.DATA.PREPROCESS.LABEL_TYPE等
#     Returns:
#         dict: {subj: MAE}
#     """
#     subject_mae_dict = {}
...
#             diff_flag = False
#             if hasattr(config, 'TEST') and hasattr(config.TEST.DATA.PREPROCESS, 'LABEL_TYPE'):
#                 if config.TEST.DATA.PREPROCESS.LABEL_TYPE == "DiffNormalized":
#                     diff_flag = True
#             fs = config.TEST.DATA.FS if hasattr(config.TEST.DATA, 'FS') else 30
#             hr_label, hr_pred, _ = calculate_metric_per_video(pred, label, fs=fs, diff_flag=diff_flag, use_bandpass=True, hr_method='FFT')
#             pred_hrs.append(hr_pred)
#             label_hrs.append(hr_label)
#         pred_hrs = np.array(pred_hrs)
#         label_hrs = np.array(label_hrs)
#         mae = np.mean(np.abs(pred_hrs - label_hrs))
#         subject_mae_dict[subj] = mae
#     return subject_mae_dict
...
#         mae = np.mean(np.abs(subj_preds - subj_labels))
#         subject_mae_dict[subj] = mae
#     return subject_mae_dict
